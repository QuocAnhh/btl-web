...
function initSearch() {
  ...
  if (searchIcon && searchInput) {
    ...
    // Tìm kiếm khi click vào container (backup)
    if (searchContainer) {
...
// Khởi tạo search khi DOM load
document.addEventListener("DOMContentLoaded", function () {
  initSearch();
});

// Backup: Khởi tạo search khi window load
window.addEventListener("load", function () {
  setTimeout(initSearch, 100);
});

// Function tìm kiếm trực tiếp (backup)
